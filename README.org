...
- Tailscale reverse proxy via Caddy. This supports mobile PWA via both http://home-ai and
  https://home-ai.TAILNET_ID.ts.net.
...
** Tailscale
Go to https://login.tailscale.com/admin/settings/keys and generate an auth key.
Leave Resusable unchecked. The compose config works with a single use auth key and
and restarts using state in the Caddy state volume.

Check Tags and choose a tag, e.g. "web server". This disables node key expiry for the
device.

Save the auth key in .env.

** Create secrets
Copy .env.tmpl to .env and fill in your keys.



** Set up Open WebUI
See https://shawnhoover.dev/notes/home-ai-server.html for steps to:
- Configure AWS Bedrock and an IAM user.
- Set up connections in Open WebUI.
